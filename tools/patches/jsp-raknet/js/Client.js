...
class Client extends events_1.default {
    ...
    async connect() {
        // OpenConnectionRequest2 encodes IPv4 octets — hostnames must be resolved first
        if (this.hostname && !/^\d{1,3}(?:\.\d{1,3}){3}$/.test(this.hostname)) {
            try {
                const dns = require('dns').promises;
                const ips = await dns.resolve4(this.hostname);
                if (ips && ips[0]) {
                    this.outLog('[raknet] resolved', this.hostname, '→', ips[0]);
                    this.hostname = ips[0];
                    this.address = new InetAddress_1.default(this.hostname, this.port);
                }
            } catch (e) {
                this.emit('error', new Error('DNS resolve failed: ' + (e && e.message || e)));
                return;
            }
        }
    ...
    async handleOpenConnectionReply1(buffer) {
        ...
        if (decodedPacket.security && decodedPacket.cookie != null) {
            packet.cookie = decodedPacket.cookie;
        }
        packet.encode();
