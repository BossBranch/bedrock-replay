...
class Connection extends EventEmitter {
  ...
  sendMCPE (buffer, immediate) {
    ...
    try {
      ...
      // Rate-limit — flood of -3 during backpressure stalls the loop worse via logging
      const now = Date.now()
      if (!this._sendFailLogAt || now - this._sendFailLogAt > 2000) {
        this._sendFailLogAt = now
        console.warn('[auth] sendReliable fail:', e?.message || e, 'len=', buffer?.length)
      }
    ...
    if (!this.batchHeader || buffer[0] === this.batchHeader) { // wrapper
      if (this.encryptionEnabled) {
        ...
        try {
          ...
          // Uncaught inflate here = login never seen → "server not found" with MOTD still OK
          console.warn(
            '[auth] frame decode fail:', e?.message || e,
            'len=', buffer?.length,
