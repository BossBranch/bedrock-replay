...
class Framer {
  ...
  static decode (client, buf) {
    ...
    if (client.features.compressorInHeader && client.compressionReady) {
      // 1.20.60+: algorithm byte in header — must not throw uncaught (drops login silently)
      try {
        decompressed = this.decompress(buffer[0], buffer.slice(1))
      } catch (e) {
        console.warn('[framer] header-decompress fail alg=', buffer[0], e?.message || e, 'len=', buffer.length)
        try {
          decompressed = this.decompress(client.compressionAlgorithm, buffer)
        } catch {
          try {
            decompressed = this.decompress(0, buffer.slice(1))
          } catch {
            decompressed = buffer
          }
        }
